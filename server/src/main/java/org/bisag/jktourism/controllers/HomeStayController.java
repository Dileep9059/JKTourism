...
import org.bisag.jktourism.services.HomeStayService;
...
@RestController
@RequestMapping("/api/home-stay")
public class HomeStayController {

    @Autowired
    HomeStayService homeStayService;

    @PostMapping("/get-home-stays")
    public ResponseEntity<String> getHomeStays(@RequestBody String req) {
        try {
            ...
            return ResponseEntity.ok().body(Json.serialize(homeStayService.getStays(page, size, sort, search, state, district)));
        } catch (Exception e) {
    ...
    public ResponseEntity<String> getDistricts(@RequestBody String req) {
        try {
            ...
            return ResponseEntity.ok().body(Json.serialize(homeStayService.getDistricts(json.get("state").asText())));
        } catch (Exception e) {
