...
public interface TravelAgentRepository extends JpaRepository<TravelAgent, Long> {
    ...
    @Query("""
            ...
            """)
    Page<TravelAgent> searchFiltered(@Param("search") String search,
            @Param("state") String state,
            @Param("district") String district,
            Pageable pageable);

    @Query("SELECT DISTINCT h.district FROM TravelAgent h WHERE LOWER(h.state) = LOWER(:state) ORDER BY h.district ASC")
