...
public interface TourGuideRepository extends JpaRepository<TourGuide, Long> {
    ...
    @Query("""
            ...
            """)
    Page<TourGuide> searchFiltered(@Param("search") String search,
            @Param("state") String state,
            @Param("district") String district,
            Pageable pageable);

    @Query("SELECT DISTINCT h.district FROM TourGuide h WHERE LOWER(h.state) = LOWER(:state) ORDER BY h.district ASC")
