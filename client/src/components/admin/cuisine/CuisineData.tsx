...
const CusineData = () => {
    ...
    const fetchCuisines = async () => {
        ...
        try {
            ...
            const res = await axiosPrivate.post(`/api/admin/get-cuisines`, encParams);
            ...
            toast("Error!", {
                description: "Something went wrong while fetching cuisines.",
    ...
    const columns: ColumnDef<CuisineData>[] = [
        {
            header: "S.No.",
        ...
        // Toogle this to enable / disbale category visibility
        {
            id: "status",
            header: "Status",
            cell: ({ row }) => {
                ...
                const handleToggle = async (checked: boolean) => {
                    try {
                        ...
                        const response = await axiosPrivate.post(
                            "/api/admin/update-cuisine-status",
                            formData
                        );
                        const message = JSON.parse(await d(response?.data?.message));
                        if (message === "Cuisine Status Updated Successfully.") {
        ...
        //   header: "Cover Image",
        //   cell: ({ row }) => {
        //     return <EncryptedImage imagePath={row.original.coverImage} />;
        //   },
        // },
    ];
    ...
    return (
        <>
            <div className="p-4">
                <h1 className="text-2xl font-bold text-[#2b5f60]">Cuisines</h1>
                <DataTable
                    columns={columns}
                    data={data}
