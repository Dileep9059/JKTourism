...
const UploadActivityData = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    images: [{ id: "1", title: "", file: null }],
    videos: [{ id: "1", title: "", file: null }],
    brochures: [{ id: "1", title: "", file: null }],
  });
  ...
  const handleFormSubmit = async (event: React.FormEvent) => {
    ...
    if (selectedActivity === "") {
      toast("Error!", {
        description: "Please select a activity",
    ...
    try {
      const response = await axiosPrivate.post(
        "/api/admin/upload-activity-files",
        formDataToSend,
        {
          headers: {
  ...
  useEffect(() => {
    async function fetchActivities() {
      try {
        ...
        toast("Error fetching activities.", {
          description: "Please try again later.",
          icon: <Info className="text-red-500 w-4 h-4" />,
        });
      }
      ...
      <div className="p-4">
        <form onSubmit={handleFormSubmit}>
          <h1 className="text-2xl font-bold text-[#2b5f60]">
            Upload data for selected Activity
          </h1>
          <div className="h-auto my-5">
            <Select onValueChange={handleActivityChange}>
              ...
              <SelectContent>
                {activities?.destinations?.map((activity: any) => (
                  <SelectItem value={activity.destination_url} key={activity.destination_url}>
                    {activity.destination_title}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
...
export default UploadActivityData;
