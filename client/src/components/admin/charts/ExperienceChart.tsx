...
export default function ExperienceChart() {
  ...
  const [chartData, setChartData] = useState<ExperienceStat[] | null>(null);

   const { theme } = useTheme();

  async function getChartData(): Promise<ExperienceStat[] | undefined> {
    try {
      const res = await axiosPrivate.post("/api/dashboard/experience-chart");
  ...
  useEffect(() => {
    setOptions({
      chart: { type: "column" },
      title: { text: "Experience  Hits" },
      xAxis: {
        categories: chartData?.map((d: ExperienceStat) => d.experience),
